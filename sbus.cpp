...
bool SbusInput::isFailsafe(void) {
    return flagByte & SBUS_STATE_FAILSAFE;
}
...
{
    _serial.begin(100000, SERIAL_8E2, 14, 25, true, 100UL);
}
...
void SbusInput::sbusToChannels(byte buffer[]) {
    ...
    setRcChannelCallback(10, mapSbusToChannel((buffer[14]>>6 | buffer[15]<<2 | buffer[16]<<10) & 0x07FF), 0);
    setRcChannelCallback(11, mapSbusToChannel((buffer[16]>>1 | buffer[17]<<7) & 0x07FF), 0);
    setRcChannelCallback(12, mapSbusToChannel((buffer[17]>>4 | buffer[18]<<4) & 0x07FF), 0);
    setRcChannelCallback(13, mapSbusToChannel((buffer[18]>>7 | buffer[19]<<1 |buffer[20]<<9) & 0x07FF), 0);
    setRcChannelCallback(14, mapSbusToChannel((buffer[20]>>2 | buffer[21]<<6) & 0x07FF), 0);
    setRcChannelCallback(15, mapSbusToChannel((buffer[21]>>5 | buffer[22]<<3) & 0x07FF), 0);
